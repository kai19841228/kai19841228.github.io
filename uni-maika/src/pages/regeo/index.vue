<template>
  <div class="address">
    <div class="search">
      ...
      <input id="input" type="text" class="searchText" :placeholder="searchtext" v-model="textSearch" @input="autoInput">
...
export default {
  ...
  computed: {
    cityUp () {
      return this.$store.state.cityUp
    },
    cityDown () {
      return this.$store.state.cityDown
    }
  },
  watch: {
  },
  methods: {
    toCity () {
      // 变更城市
      let url = '/pages/citySelect/index?city=' + this.address
      uni.navigateTo({ url })
    },
    backPage () {
      uni.navigateBack()
    ...
    addressHandle (item) {
      if (this.address === 'up') {
        this.$store.dispatch('setUpAdress', item)
      } else {
        this.$store.dispatch('setDownAdress', item)
      }
      this.backPage()
    }
  },
