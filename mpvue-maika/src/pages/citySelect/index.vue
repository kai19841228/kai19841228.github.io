<template>
  <div class="citySelect">
    ...
    <div class="zimu" v-show='!openStatu'>
      <a v-for="(citys, key) in cityDataAll" :key="key" @click="tapMove(key)">{{key}}</a>
    </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    cityChange (item) {
      ...
      // 根据 url参数 改变上下车store
      if (this.openStatu === 1) {
        store.dispatch('setUpCity', item)
        if (!store.state.order.cityDown.cityName) {
          store.dispatch('setDownCity', item)
        }
      } else {
        store.dispatch('setDownCity', item)
      }
